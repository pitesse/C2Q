...
def generate_quantum_ir(
    ast, print_generic: bool, optimize: bool = True, precision: float = 1e-4
):
...
def get_output_path(input_path: Path, suffix: str) -> Path:
    ...
    if "tests/inputs" in input_path_str or "tests\\inputs" in input_path_str:
        output_dir = Path(
            input_path_str.replace("tests/inputs", "tests/outputs").replace(
                "tests\\inputs", "tests\\outputs"
            )
        ).parent
    ...
    match path.suffix:
        case ".c":
            ...
            try:
                ...
                print(
                    "This may be due to the structure of the quantum operations in the IR."
                )
                return None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ir", dest="ir", action="store_true", help="Print the generated IR to console"
    )
    parser.add_argument(
        "--print-op-generic",
        dest="print_generic",
    ...
    if args.validate is not None and not args.force_optimize:
        ...
        if not args.no_optimize:
            print(
                "[WARN] Optimizations disabled for validation (use --force-optimize to test optimized circuits)"
            )
