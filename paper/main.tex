...
\twocolumn[{
      ...
      \begin{abstract}
        This report presents \textbf{C2Q}, a complete compilation framework
        ...
        and features a optimization framework that achieves 14--56\% gate
...
sequences which limits accessibility to quantum computing.
...
for arithmetic-intensive algorithms. Rather than manual quantum circuit design the compiler automates the translation process, ensuring correctness while applying optimizations
...
\begin{itemize}
  ...
  \item Multiplication via doubly-controlled phase accumulation
...
\begin{enumerate}
  ...
  \item An integrated optimization framework achieving 34.9\% average gate reduction, 42.1\% average depth reduction,
        and 34.6\% MLIR operation reduction across the benchmark suite.
...
The remainder of this report is organized as follows: Section II describes
...
\begin{itemize}
  ...
  \item \textbf{Single-qubit gates:} \texttt{HadamardOp}, \texttt{PhaseOp}, \texttt{NotOp}
  \item \textbf{Multi-qubit gates:} \texttt{OnQubitCNotOp}, \texttt{OnQubitCCnotOp}, \texttt{OnQubitControlledPhaseOp}
  \item \textbf{Measurement:} \texttt{MeasureOp} extracts classical results
  \item \textbf{Metadata:} \texttt{CommentOp} for IR comments
\end{itemize}
...
\begin{lstlisting}[language=Python, caption={QFT Implementation}]
def qft_on_vector(builder, vec):
  n = vector_width(vec)
  # Matches current implementation: high index -> low index
  for i in reversed(range(n)):
    ...
    for j in reversed(range(i)):
      ...
      vec = OnQubitControlledPhaseOp(
        vec, j, i, angle)
  # no final swap in this implementation
  return vec
\end{lstlisting}
...
In the Fourier basis, addition becomes phase multiplication. For each bit $a_i$ of the first
operand, we apply controlled phase rotations to target bits $j \ge i$ of the second operand:
...
Multiplication is implemented by accumulating doubly-controlled phase rotations directly in Fourier space. The product register
is first transformed with QFT, then each bit-pair $(a_i, b_j)$ contributes phase to product bits $k \ge i+j$:

\begin{lstlisting}[language=Python, caption={Multiplication via CCPhase Accumulation}]
def quantum_multiply(builder, a, b):
    width = vector_width(a) + vector_width(b)
    prod = InitOp(VectorType(IntegerType(1), [width]))
    prod = qft_on_vector(builder, prod)

    for i in range(vector_width(a)):
        for j in range(vector_width(b)):
            for k in range(i + j, width):
                angle = 2*pi / 2**(k - (i + j) + 1)
                prod = CCPhase(builder, a, i, b, j, prod, k, angle)

    return iqft_on_vector(builder, prod)
\end{lstlisting}

For small operands (e.g., $2 \times 3$), the base circuit contains approximately 3,155 gates and depth 2,051.
For balanced widths ($n_a \approx n_b \approx n$), the implemented triple loop gives $O(n^3)$ phase-accumulation complexity.
...
Multiplication circuits benefit most due to cumulative high-precision rotations from nested CCPhase accumulation.
...
This report presented C2Q, a complete compiler infrastructure for translating classical C programs
...
arithmetic algorithms, or language features. I hope this contributes to the broader effort to make
quantum computing more accessible to software developers without quantum expertise.


\subsection{Paradigm Reflections and Future Language Design}

While C2Q successfully demonstrates the feasibility of translating a classical procedural language into quantum circuits,
the development process highlighted a fundamental paradigm mismatch between classical von Neumann architectures and quantum mechanics.

The C language is deeply rooted in the concepts of mutable state, sequential memory overwrites, and unconstrained variable duplication.
In the quantum realm, these concepts clash directly with physical constraints: overwriting data requires costly uncomputation to maintain
coherence (due to Landauer's principle), and copying quantum states is physically impossible due to the No-Cloning Theorem. While the compiler
mitigates these issues through Static Single Assignment (SSA) and Draper's phase-space arithmetic, forcing a procedural language to behave
quantum-mechanically introduces unavoidable overhead.

For future research in quantum compiler design, I hypothesize that functional languages (such as Haskell) and languages with
linear type systems (such as Rust) represent a much more natural fit for quantum abstractions.
Functional languages rely on immutability and pure function evaluation, which perfectly mirrors the application of unitary, reversible
transformations to quantum statevectors. Furthermore, the strict ownership and borrowing model of Rust, where a variable cannot be implicitly
copied once its ownership is moved, acts as a perfect software-level analogue to the No-Cloning Theorem.

Ultimately, while translating legacy C code is a valuable bridging exercise, the future of native quantum software engineering will likely
rely on paradigms that enforce quantum physical constraints directly at the compile-time type level.
