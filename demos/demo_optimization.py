...
def run_comparison(test_file: str, description: str):
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            cwd="/home/linuxesse/C_to_Quantum",
        )
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if "gates_before" in metrics and "gates_after" in metrics:
                ...
                print(
                    f"   [INFO] gate reduction: {gate_improvement} ({gate_percent:.1f}%)"
                )
            ...
            if "depth_before" in metrics and "depth_after" in metrics:
                ...
                print(
                    f"   [INFO] depth reduction: {depth_improvement} ({depth_percent:.1f}%)"
                )
            ...
            if "width_before" in metrics and "width_after" in metrics:
                ...
                print(
                    f"   [INFO] width reduction: {width_improvement} ({width_percent:.1f}%)"
                )
        else:
            print(f"   [ERROR] error: {result.stderr}")
