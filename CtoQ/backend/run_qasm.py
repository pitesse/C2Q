...
def generate_quantum_truth_table(
    first_op, qubit_number, init_number, output_number, istates
):
    counts = {}
    quantum_truth_table = {}
...
for i in range(4):
    ...
    quantum_truth_table = generate_quantum_truth_table(
        first_op,
        info["qubit_number"],
        info["init_number"],
        info["output_number"],
    ...
    if valid:
        ...
        for key in quantum_truth_table:
            if quantum_truth_table[key] != classical_truth_table.get(key):
                print(
                    f"Difference found at input {key}: quantum circuit returns {quantum_truth_table[key]}, "
                )
